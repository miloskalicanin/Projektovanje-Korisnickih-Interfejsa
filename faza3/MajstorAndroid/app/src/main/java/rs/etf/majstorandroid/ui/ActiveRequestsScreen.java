...
import rs.etf.majstorandroid.recycler.ListRecyclerAdapter;
import rs.etf.majstorandroid.recycler.RecyclerViewBinder;
import rs.etf.majstorandroid.recycler.RequestItemHolderFactory;
...
public class ActiveRequestsScreen extends Fragment {
    ...
    private void onBindItem(final RequestItemHolderFactory.ItemViewHolder holder,
        ...
        if (item.isUrgent()) {
            holder.icon.setImageResource(R.drawable.ic_clock_fast_black_24dp);
            holder.icon.setVisibility(View.VISIBLE);
        } else {
            holder.icon.setVisibility(View.GONE);
        }

        UserDTO client = item.getClient();
